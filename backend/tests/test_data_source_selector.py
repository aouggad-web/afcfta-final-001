...
class TestSourceSelection:
    ...
    async def test_fallback_to_oec_when_comtrade_fails(self):
        ...
        selector.comtrade.get_bilateral_trade = AsyncMock(side_effect=Exception("API Error"))
    ...
    async def test_no_data_from_any_source(self):
        ...
        selector.comtrade.get_bilateral_trade = AsyncMock(side_effect=Exception("Error"))
        if selector.oec:
            selector._get_oec_data = AsyncMock(return_value=None)
...
class TestErrorHandling:
    ...
    async def test_wto_api_error_handling(self):
        ...
        # Check that error was recorded
        wto_check = next(
            (c for c in result["sources_checked"] if c["source"] == "WTO"),
            None
        )
        if wto_check:
...
class TestCompareDataSources:
    ...
    async def test_compare_data_sources(self):
        ...
        assert len(result["countries_checked"]) <= 5  # Limited to 5 to avoid rate limits
